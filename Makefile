...
index:
	@echo "Make: docker-compose run --rm web bash -c 'Dockers/wait-for db:5432 && Dockers/wait-for es_old:9200 && python manage.py index -f'"
	@echo ""
	@docker-compose run --rm web bash -c 'Dockers/wait-for db:5432 && Dockers/wait-for es_old:9200 && python manage.py index -f'
	@echo ""
