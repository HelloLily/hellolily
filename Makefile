...
index:
	@echo "Make: docker-compose run --rm web bash -c 'Dockers/wait-for db:5432 && Dockers/wait-for es_old:9200 && Dockers/wait-for es:9200 && python manage.py index -f && python manage.py search_index rebuild -f'"
	@echo ""
	@docker-compose run --rm web bash -c 'Dockers/wait-for db:5432 && Dockers/wait-for es_old:9200 && Dockers/wait-for es:9200 && python manage.py index -f && python manage.py search_index rebuild -f'
	@echo ""

test:
	@echo "Make: docker-compose run --rm -e ES_OLD_DISABLED=1 web bash -c 'Dockers/wait-for db:5432 && python manage.py test'"
	@echo ""
	@docker-compose run --rm -e ES_OLD_DISABLED=1 web bash -c 'Dockers/wait-for db:5432 && python manage.py test'
	@echo ""
