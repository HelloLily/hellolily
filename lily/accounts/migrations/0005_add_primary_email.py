...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        For each account, check if there is no primary email address set. If not set, make first found primary.
        """
        ...
        for account in orm['accounts.Account'].objects.all():
            try:
                account.email_addresses.get(is_primary=True)
            except:
                try:
                    email_address = account.email_addresses.all()[0]
