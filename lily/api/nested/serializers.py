...
class WritableNestedSerializerMetaclass(SerializerMetaclass):
    def __new__(mcs, name, bases, attrs):
        ...
        if name not in ['WritableNestedSerializer', 'NewWritableNestedSerializer']:
...
@six.add_metaclass(WritableNestedSerializerMetaclass)
class NewWritableNestedSerializer(serializers.ModelSerializer):
    simple_data = {}
    fk_data = {}
    fk_reverse_data = {}
    gfk_data = {}
    ...
    def __init__(self, instance=None, data=empty, **kwargs):
        super(NewWritableNestedSerializer, self).__init__(instance=instance, data=data, **kwargs)
    ...
    def save(self, **kwargs):
        # Get the content type for the model of the current serializer.
        self.ctype = ContentType.objects.get_for_model(self.Meta.model)
        return super(NewWritableNestedSerializer, self).save(**kwargs)
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            ...
            # Save the instance using simple field data.
            self.instance = super(NewWritableNestedSerializer, self).create(self.simple_data)
    ...
    def update(self, instance, validated_data):
        ...
        with transaction.atomic():
            ...
            # Save the instance.
            self.instance = super(NewWritableNestedSerializer, self).update(instance, self.simple_data)
            self.elasticsearch_actions.add_model_actions(self.instance)
