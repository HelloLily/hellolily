...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # For every current tenant lets create a least one status.
        for tenant in orm['tenant.Tenant'].objects.all():
            orm.CaseStatus.objects.create(tenant=tenant, position=1, status='New')

            # And set the status for every case
            status = orm.CaseStatus.objects.get(tenant=tenant, position=1)
            for case in orm.Case.objects.filter(tenant=tenant):
                case.status = status
                case.save()
    ...
    models = {
        ...
        u'cases.case': {
            ...
            'status': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'cases'", 'null': 'True', 'to': u"orm['cases.CaseStatus']"}),
