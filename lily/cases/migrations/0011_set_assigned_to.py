...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for case in orm.Case.objects.all():
            customuser_email = case.assigned_to.contact.email_addresses.filter(is_primary=True).first().email_address
            case.assigned_to2 = orm['users.LilyUser'].objects.get(email=customuser_email)
            case.save()
