...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        For each contact, check if there is no primary email address set. If not set, make first found primary."
        """
        # Note: Don't use "from appname.models import ModelName".
        # Use orm.ModelName to refer to models in this application,
        # and orm['appname.ModelName'] for models in other applications.
        for contact in orm['contacts.Contact'].objects.all():
            try:
                contact.email_addresses.get(is_primary=True)
            except:
                try:
                    email_address = contact.email_addresses.all()[0]
