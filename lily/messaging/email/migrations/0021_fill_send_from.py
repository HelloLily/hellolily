...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        email_outbox_messages = orm['email.EmailOutboxMessage'].objects.all()

        for email_outbox_message in email_outbox_messages:
            if email_outbox_message.from_email:
                email_account = orm['email.EmailAccount'].objects.get(email__email_address=email_outbox_message.from_email)
                email_outbox_message.send_from = email_account
    ...
    def backwards(self, orm):
        ...
        for email_outbox_message in email_outbox_messages:
            email_outbox_message.send_from = None
            email_outbox_message.save()
    ...
    models = {
        ...
        'email.emailoutboxmessage': {
            ...
            'send_from': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'outbox_messages'", 'null': 'True', 'to': "orm['email.EmailAccount']"}),
            'subject': ('django.db.models.fields.CharField', [], {'max_length': '255', 'null': 'True', 'blank': 'True'}),
            'tenant': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['tenant.Tenant']", 'blank': 'True'}),
