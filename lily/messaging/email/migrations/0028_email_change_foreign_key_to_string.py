...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for emailaccount in orm.EmailAccount.objects.all():
            try:
                # Only convert accounts with foreign key references, not actual email addresses
                email_pk = int(emailaccount.email)
            except:
                continue

            try:
                emailaddress = orm['utils.EmailAddress'].objects.get(pk=email_pk)
            except:
                continue

            emailaccount.email = emailaddress.email_address
            emailaccount.save()

    def backwards(self, orm):
        for emailaccount in orm.EmailAccount.objects.all():
            try:
                emailaddress = orm['utils.EmailAddress'].objects.get(email_address=emailaccount.email)
            except:
                continue

            emailaccount.email = emailaddress.pk
            emailaccount.save()
