...
class Migration(SchemaMigration):
    ...
    depends_on = (
        ('utils', '0007_fill_historylist_tenant_id_with_proper_values'),
    )

    def forwards(self, orm):
        # Deleting field 'Message.tenant'
        db.delete_column('messaging_message', 'tenant_id')


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'Message.tenant'
        raise RuntimeError("Cannot reverse this migration. 'Message.tenant' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'Message.tenant'
        db.add_column('messaging_message', 'tenant',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['tenant.Tenant'], blank=True),
                      keep_default=False)
