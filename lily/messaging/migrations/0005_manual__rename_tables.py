...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        Rename messages to messaging
        """
        if not MigrationHistory.objects.filter(app_name='messages').exists():
            return

        # Rename tables
        if 'messages_messagesaccount' in connection.introspection.table_names():
            db.rename_table('messages_messagesaccount', 'messaging_messagesaccount')

        if 'messages_messagesaccount_user_group' in connection.introspection.table_names():
            db.rename_table('messages_messagesaccount_user_group', 'messaging_messagesaccount_user_group')

    def backwards(self, orm):
        """
        Rename messaging to messages
        """
        if not MigrationHistory.objects.filter(app_name='messages').exists():
            return

        # Rename tables
        if 'messaging_messagesaccount' in connection.introspection.table_names():
            db.rename_table('messaging_messagesaccount', 'messages_messagesaccount')

        if 'messaging_messagesaccount_user_group' in connection.introspection.table_names():
            db.rename_table('messaging_messagesaccount_user_group', 'messages_messagesaccount_user_group')
