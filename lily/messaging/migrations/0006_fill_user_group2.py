...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        Fill up the new user_group2 field on MessagesAccount so we can migrate towards LilyUser.
        """
        for account in orm.MessagesAccount.objects.all():
            for customuser in account.user_group.all():
                user_primary_email = customuser.contact.email_addresses.filter(is_primary=True).first()
                if not user_primary_email:
                    print 'customuser with pk: %s is missing a primary email address, therefore it cannot be migrated' % customuser.pk
                lilyuser = orm['users.LilyUser'].objects.filter(email=user_primary_email.email_address).first()

                if lilyuser:
                    account.user_group2.add(lilyuser)

    def backwards(self, orm):
        """
        Fill up the old user_group field on MessagesAccount so we can migrate back to CustomUser.
        """
        for account in orm.MessagesAccount.objects.all():
            for lilyuser in account.user_group2.all():
                customuser = orm['users.CustomUser'].objects.filter(
                    contact__email_addresses__is_primary=True,
                    contact__email_addresses__email_adress=lilyuser.email,
                ).first()

                if customuser:
                    account.user_group.add(customuser)
    ...
    models = {
        ...
        u'messaging.messagesaccount': {
            ...
            'tenant': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['tenant.Tenant']", 'blank': 'True'}),
            'user_group': ('django.db.models.fields.related.ManyToManyField', [], {'related_name': "'messages_accounts_shared'", 'symmetrical': 'False', 'to': "orm['users.CustomUser']"}),
            'user_group2': ('django.db.models.fields.related.ManyToManyField', [], {'related_name': "'messages_accounts_shared'", 'symmetrical': 'False', 'to': u"orm['users.LilyUser']"})
        },
        u'tenant.tenant': {
