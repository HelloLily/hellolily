...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        Set the owner of all MessagesAccounts to the first user attached to it, and clear the user_group (no sharing).
        """
        PRIVATE, PUBLIC, SHARED = range(3)

        for messages_account in orm.MessagesAccount.objects.all():
            user = messages_account.user_group.first()
            if user:
                messages_account.owner_id = user.pk
                messages_account.user_group.clear()
                messages_account.shared_with = PRIVATE
                messages_account.save()

    def backwards(self, orm):
        """
        Set the owner back in the user_group.
        """
        for messages_account in orm.MessagesAccount.objects.all():
            messages_account.user_group.add(messages_account.owner)
    ...
    models = {
        ...
        u'messaging.messagesaccount': {
            ...
            'owner': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'messages_accounts_owned'", 'null': 'True', 'to': u"orm['users.LilyUser']"}),
