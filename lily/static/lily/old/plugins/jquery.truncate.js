/*
This jQuery.truncate is based on https://github.com/tbasse/jquery-truncate

Slight modifications:

It looks at the parent element to determine maximum size to use it in tables.
It stores the original value, so on resize, the text can grow again.
    ...
    $.fn.truncate = function (options) {
        ...
        return this.each(function () {
            ...
            if($element.data("original-text")) {
                elementText = $element.data("original-text");
            } else {
                elementText = $element.text();
                $element.data("original-text", elementText);
            }
            if (!options.assumeSameStyle) {
                fontCSS = {
                    'fontFamily': $element.css('fontFamily'),
            ...
            var truncateWidth = parseInt(options.width, 10) || $element.parent().width();
            var dimension = 'width';
