    ...
    $.fn.smartWizard = function(action) {
        ...
        return this.each(function(){
                ...
                if (!action || action === 'init' || typeof action === 'object') {
                  ...
                  return true;
                }else if (action === 'doForwardProgress') {
                  doForwardProgress();
                  return true;
                }else if (action === 'doBackwardProgress') {
                  doBackwardProgress();
                  return true;
                } else {
                ...
                function LoadContent(stepIdx){
                    ...
                    if(ajaxurl && ajaxurl.length>0){
                       if(options.contentCache && hasContent){
                          ...
                          // get form data
                          var data = $('form', obj).serialize();
                          if(!data.length) {
                            data = 'wizard_goto_step=' + stepNum;
                          } else {
                            data += '&wizard_goto_step=' + stepNum;
                          }
                          $.ajax({
                            ...
                            data: data,
                            dataType: "text",
                            beforeSend: function(jqXHR, settings){ loader.show(); addCSRFHeader(jqXHR, settings); },
                            error: function(){loader.hide();},
                            success: function(res){
                              loader.hide();
                              if(res && res.length>0){
                                 // test for 'field_error'
                                 var result = /field_error/g.test(res);
                                 if(result) {
                                  // don't progress
                                  stepIdx = curStepIdx;
