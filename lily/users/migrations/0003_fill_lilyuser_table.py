...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        Copy all CustomUsers to LilyUser table with the correct data.
        """
        for user in orm.CustomUser.objects.all():
            # Get the primary email from the contact, without it no user can be created.
            # The frontend was bugged for a while, so there can be multiple, we take the first one for simplicity.
            user_primary_email = user.contact.email_addresses.filter(is_primary=True).first()

            if not user_primary_email:
                # The contact didn't have a primary email, so it doesn't need to be migrated, it's broken already.
                continue

            try:
                # try and create the user, it would be weird for this to fail.
                lilyuser = orm.LilyUser.objects.create(
                    tenant=user.tenant,
                    is_superuser=user.is_superuser,
                    password=user.password,
                    last_login=user.last_login,
                    first_name=user.contact.first_name,
                    preposition=user.contact.preposition,
                    last_name=user.contact.last_name,
                    email=user_primary_email.email_address,
            ...
            try:
                # Add all permissions to new user.
                for permission in user.user_permissions.all():
                    lilyuser.user_permissions.add(permission)
            except Exception as e:
                print e
                continue

            try:
                # Add all groups to new user.
                for group in user.groups.all():
                    lilyuser.groups.add(group)
            except Exception as e:
                print e
                continue
    ...
    def backwards(self, orm):
        """
        Clear the LilyUser table of all contents.
        """
        orm.LilyUser.objects.all().delete()
