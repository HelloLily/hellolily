...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Update tenant_id's for all messages
        db.execute('UPDATE utils_historylistitem SET tenant_id = messaging_message.tenant_id FROM messaging_message WHERE utils_historylistitem.id = messaging_message.historylistitem_ptr_id;')

        # Set the generic foreign key for notes
        gfk = generic.GenericForeignKey('content_type', 'object_id')
        gfk.contribute_to_class(orm['notes.Note'], 'subject')

        # Update tenant_id's for all notes
        note_list = orm['notes.Note'].objects.all()
        for note in note_list:
            note.tenant_id = note.subject.tenant_id
            note.save()

    def backwards(self, orm):
        # Update tenant_id's for all messages
        db.execute('UPDATE messaging_message SET tenant_id = utils_historylistitem.tenant_id FROM utils_historylistitem WHERE utils_historylistitem.id = messaging_message.historylistitem_ptr_id;')
